...
## v0.6.x — Webview input UX 회복 (베타 hardening, dogfood-driven)

웹뷰 베타에서 v0.5.x 터미널 모드가 이미 제공하던 입력 편의 기능들이 회귀됨. dogfood
일상 흐름을 차단하므로 v0.6.0 GA 전에 채워야 함. 별도 v0.6.0-beta.2 patch 후보.

### [ ] [높음] Enter-to-send + 입력 편의 기능 회복

**What:** 사용자 dogfood 발견 (2026-04-29):
- 현재 webview의 input bar는 plain Enter 가 newline 이고 Cmd/Ctrl+Enter 로만 전송됨
  (`src/webview/ui/input-bar.ts:9-12`). 매 메시지마다 modifier 의식해야 해 일상 사용
  차단.
- v0.4.0 에서 추가됐던 `@-mention` 파일 picker 가 webview 에서 회귀 (터미널 모드만
  남음).
- `/` slash command 메뉴 (TODO 백로그) 도 부재.

**How (제안):**
- **Enter-to-send 기본 동작**: plain Enter → submit, Shift+Enter → newline (chat UX
  표준). Cmd/Ctrl+Enter 는 백워드 호환으로 유지.
- **`@` 트리거 파일 picker**: textarea 에서 `@` 입력 시 SuggestModal 또는 popover
  띄워 vault 노트 fuzzy 검색 → 선택 시 textarea 에 `[[basename]]` 또는 절대 경로
  삽입. v0.4.0 의 `at-mention.ts` 로직 재사용 가능 (터미널용 OSC 변환 부분만 빼고
  순수 picker).
- **`/` 트리거 슬래시 메뉴**: textarea 첫 글자가 `/` 일 때 `/compact`, `/clear`,
  `/mcp`, `/plugins` + `.claude/commands/*.md` 사용자 정의 커맨드 SuggestModal.
  실행 시 stdin 으로 그대로 write (Claude CLI 가 처리).
- 기존 textarea 자동 리사이즈 + `dispose` 정리 패턴 유지.

**Why 높음:**
- Enter-to-send: 매 메시지 마다 사용자 마찰. dogfood 가능성을 직접 깎음.
- @-mention: v0.4.0 부터 1년+ 굳어진 사용자 흐름. webview default flip 결정 (v0.7.0
  GA) 의 전제 조건.
- v0.6.0 GA 전에 들어가야 webview 가 터미널 대비 후퇴가 아닌 진보로 사용자에게
  인식됨.

**Files:** `src/webview/ui/input-bar.ts`, `src/webview/ui/at-mention-picker.ts` (신규),
`src/webview/ui/slash-menu.ts` (신규), `test/webview/input-bar.test.ts` 갱신.

**Effort:** ~2-3일 CC time.
- Enter-to-send: ~30분 (keydown 로직 분기만)
- @-mention: ~1일 (v0.4.0 로직 포팅 + webview-friendly DOM)
- Slash menu: ~1일

**Depends on:** 없음. 이미 들어간 webview 인프라 위에 바로 올라감.
